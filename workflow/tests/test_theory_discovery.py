...
class TheoryDiscoveryProjectTest(unittest.TestCase):
    ...
    def test_legacy_validation_does_not_complete_a_compiler_specification(self) -> None:
        self.import_graph()
        _, prediction = self.add_theory_and_prediction()
        self.freeze_measurements(prediction.id)
        self.project.add_falsification_test(
        ...
        self.project.record_validation(
            prediction.id,
            "no_evidence",
            "Legacy pilot output.",
            [],
        ...
        self.project.configure_validation_execution(True)
        self.assertEqual(
            self.project.next_action()["action"], "measure_validation_records"
        )
    ...
    def test_v4_projects_migrate_to_scoped_laws(self) -> None:
        legacy_root = Path(self.temporary_directory.name) / "legacy-study"
        legacy_root.mkdir()
        legacy_state = {
            "schema_version": 4,
            "revision": 3,
            "config": {"study_id": "legacy", "question": "A legacy question."},
            "graph_nodes": [
                {"id": "source", "label": "Source", "description": "", "examples": []},
                {"id": "outcome", "label": "Outcome", "description": "", "examples": []},
            ],
            "graph_edges": [],
            "relationships": [
                {
            ...
            "theories": [
                {
                    "id": "theory_1",
                    "name": "Legacy theory",
                    "law": "Source changes outcome.",
                    "scope": "The legacy study population.",
                    "mechanism": "The legacy proposed mechanism.",
                    "evidence": [
                        {
                            "id": "legacy_evidence",
                            "source": "Legacy source",
                            "summary": "Legacy source-backed summary.",
                            "role": "supporting",
                            "locator": "section 1",
                        }
                    ],
                    "relationship_ids": ["relationship_1"],
                    "created_at": "2026-01-01T00:00:00+00:00",
                }
            ],
            "predictions": [
                {
                    "id": "prediction_1",
                    "theory_id": "theory_1",
                    "statement": "Outcome differs when source is present.",
                    "claim_type": "associational",
                    "condition_or_exposure": "Source is present.",
                    "expected_outcome": "Outcome is observed.",
                    "comparison": "Source is absent.",
                    "expected_pattern": "More outcome when source is present.",
        ...
        (legacy_root / "state.json").write_text(json.dumps(legacy_state), encoding="utf-8")

        migrated = TheoryDiscoveryProject.open(legacy_root)

        self.assertEqual(migrated.state.schema_version, 13)
        self.assertEqual(migrated.state.evidence_records[0].id, "legacy_evidence")
        self.assertEqual(
            migrated.state.theories[0].theory_statements[0].id, "theory_1_law_1"
        )
        self.assertEqual(migrated.state.predictions[0].law_id, "theory_1_law_1")

    def test_v10_unrun_qwen38_contracts_are_superseded_before_measurement(self) -> None:
        self.import_graph()
        _, prediction = self.add_theory_and_prediction()
        self.freeze_measurements(prediction.id)
        legacy_state = self.project.state.to_dict()
        legacy_state["schema_version"] = 10
        for specification in legacy_state["measurement_specifications"]:
            specification["model_id"] = "qwen3.8-27b"
            specification["model_revision"] = "qwen3.8-27b"
        (self.root / "state.json").write_text(
            json.dumps(legacy_state), encoding="utf-8"
        )

        migrated = TheoryDiscoveryProject.open(self.root)

        self.assertEqual(migrated.state.schema_version, 13)
        self.assertEqual(migrated.state.measurement_specifications, [])
        self.assertEqual(len(migrated.state.superseded_measurement_specifications), 2)
        self.assertEqual(
            migrated.next_action()["action"], "generate_measurement_specifications"
        )

    def test_v11_unrun_contracts_are_superseded_for_explicit_groups(self) -> None:
        self.import_graph()
        _, prediction = self.add_theory_and_prediction()
        self.freeze_measurements(prediction.id)
        self.project.add_falsification_specification(
        ...
        legacy_state = self.project.state.to_dict()
        legacy_state["schema_version"] = 11
        (self.root / "state.json").write_text(
            json.dumps(legacy_state), encoding="utf-8"
        )
        ...
        self.assertEqual(migrated.state.schema_version, 13)
        self.assertEqual(migrated.state.measurement_specifications, [])
        self.assertEqual(migrated.state.falsification_specifications, [])
        self.assertEqual(len(migrated.state.superseded_measurement_specifications), 2)
        self.assertEqual(len(migrated.state.legacy_falsification_specifications), 1)
        self.assertEqual(
            migrated.next_action()["action"], "generate_measurement_specifications"
        )
